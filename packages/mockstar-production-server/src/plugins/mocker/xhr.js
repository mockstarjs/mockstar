...
module.exports = (router, mockerParser, localServerConfig) => {
    let { namespaceList } = localServerConfig;

    // 根据用户配置的路由关系，进行解析
    // console.log('mockerList', mockerList);
    namespaceList.forEach((namespaceItem) => {
        let mockerList = mockerParser.getNamespaceAllMocker(false, namespaceItem);
...
function handleCallback(req, res, next, opts = {}) {
    ...
    const { routePath, mockerItem, mockerParser, namespaceItem } = opts;
    ...
    if (mockstarQueryItem) {
        ...
        if (!isDisabled) {
            ...
            let curMockerItem = mockerParser.getNamespaceMockerByName(mockerItem.name, true, namespaceItem);
    ...
    if (isDisabled) {
        ...
        const resInfo = mockerParser.getNamespaceResInfoByRoute(routePath, params, namespaceItem);
